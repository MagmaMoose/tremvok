...
| `require-access` | `boolean` | `False` | Refuse to deploy unless a Cloudflare Access application already covers the site's hostname. Set true wherever the docs must not be world-readable. The application is created in the Cloudflare dashboard, not by any API: Access rejects a pages.dev hostname as "domain does not belong to zone". |
