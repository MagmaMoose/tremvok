...
`MagmaMoose/tremvok@v2` takes 125 inputs. `target` is the only one that
is required.
...
| `apim-policy-format` | `azure-apim-policy` | `rawxml` | azure-apim-policy: how the documents are written. `rawxml` (default) is what the portal's editor shows: policy expressions unescaped, `@(context.Request.Headers.GetValueOrDefault("x", ""))` as it reads. `xml` is strict XML: every quote and angle bracket inside an expression must be entity-escaped. `rawxml` takes an expression either way, so it is the safe default. A document API Management cannot compile is refused at publish, because it compiles every expression then. |
