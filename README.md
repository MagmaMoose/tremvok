...
- **Seven targets, one action**: `github-pages`, `cloudflare-workers`,
  `azure-functions-zip`, `s3-cloudfront`, `lambda-zip`, `terragrunt`, `ansible`. `target` is
...
| --- | --- | --- |
| `target` | — | `github-pages` · `cloudflare-workers` · `azure-functions-zip` · `s3-cloudfront` · `lambda-zip` · `terragrunt` · `ansible`. Required. |
